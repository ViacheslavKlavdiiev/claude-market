...
**Do NOT flag**: test files, dead code, server-controlled values (env vars, config constants), framework-mitigated patterns (view-layer auto-escaping, ORM parameterized queries), development-only code gated by `NODE_ENV`.
...
- Client-side route guards are UX only — **server must enforce all access control**
...
- Your view layer's default auto-escaping (e.g., JSX-style templating) is your default safety net
...
- Raw-HTML injection APIs (e.g., `dangerouslySetInnerHTML`) bypass auto-escaping — require DOMPurify or an equivalent sanitizer
- `href={userUrl}` allows `javascript:` XSS — validate protocol
- Build-time public env vars (e.g., `VITE_*`) are public — never prefix secrets
